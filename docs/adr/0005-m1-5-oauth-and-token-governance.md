...
- 現状との差分: M0 時点ではヘッダー認証すら未接続の公開状態で `/healthz` と `/metrics` が無認証アクセス可能だったが、M1 で `Authorization: Bearer <CHAINLIT_AUTH_SECRET>` による保護を導入済み。OAuth 適用後は公開エンドポイントの整理と追加ガード導入が必須となる。具体的な作業は `tasks/TASK.2025-10-19-0002.md` にて追跡する。
