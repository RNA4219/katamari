...
import {
  ...
  SOCKET_IO_BACKOFF_FACTOR,
  SOCKET_IO_RECONNECTION_ATTEMPTS
} from './useChatSession';
...
describe('useChatSession', () => {
  it('retries websocket connection with exponential backoff configuration', async () => {
    ...
    const expectedMaxDelay =
      SOCKET_IO_RECONNECTION_OPTIONS.reconnectionDelay *
      SOCKET_IO_BACKOFF_FACTOR **
        (SOCKET_IO_RECONNECTION_ATTEMPTS - 1);
    expect(options.reconnectionDelayMax).toBe(expectedMaxDelay);
  });
});
